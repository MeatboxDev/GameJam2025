...
func _on_focus() -> void:
	...
	_interaction_info_tween = get_tree().create_tween().set_ease(Tween.EASE_OUT).set_trans(
		Tween.TRANS_BACK
	)
...
func _on_focus_lost() -> void:
	_interaction_info_tween = get_tree().create_tween().set_ease(Tween.EASE_IN).set_trans(
		Tween.TRANS_BACK
	)
	_interaction_info_tween.tween_property(_interaction_info, "scale", Vector3.ONE * 0.01, 0.33)
	_interaction_info_tween.tween_callback(func() -> void: _interaction_info.visible = false)
